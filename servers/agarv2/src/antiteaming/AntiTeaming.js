...
class AntiTeaming {
	...
	onCellEaten(eatenCell, eaterCell) {
		...
		// Detect both player cell feeding (type 0) and ejected mass feeding (type 3)
		if ((eatenCell.type === 0 || eatenCell.type === 3) && eaterCell.type === 0) {
			const victim = eatenCell.owner;
			const eater = eaterCell.owner;

			if (!victim || !eater || victim.id === eater.id) return; // Same player or invalid, not teaming
	...
	getMassAbsorptionMultiplier(playerId, eatenCell = null) {
		...
		// Only apply penalty to player cells (type 0) and ejected mass (type 3), not pellets, viruses, etc.
		if (!eatenCell || (eatenCell.type !== 0 && eatenCell.type !== 3)) {
			return 1.0;
		}

		// Don't penalize eating your own cells (merging/recombining) or your own ejected mass
		...
		if (isCurrentlyUnderPenalty) {
			...
			if (player) {
				...
				if (currentTick - lastLogTime > 750) { // 30 seconds at 25 TPS
					this.logger.inform(`STEALTHY PUNISHMENT ACTIVE: Player ${playerId} mass absorption from player cells and ejected mass reduced to ${this.settings.antiTeamingMassAbsorptionPenalty * 100}% (suspicion: ${data.suspicionLevel.toFixed(1)})`);
				...
				if (this.settings.antiTeamingStealthyMessage) {
					this.sendMessageToPlayer(player, "⚠️ Anti-teaming: Your mass absorption from player cells and ejected mass is reduced due to suspected teaming behavior.", "stealthy");
				}
			}
			...
			if (wasUnderPenalty) {
				...
				if (player) {
					...
					this.logger.inform(`STEALTHY PUNISHMENT CLEARED: Player ${playerId} mass absorption from player cells and ejected mass restored to 100% (suspicion: ${data.suspicionLevel.toFixed(1)})`);
					
					// Send message only if enabled and this is the first time we're clearing
					if (this.settings.antiTeamingStealthyMessage) {
						this.sendMessageToPlayer(player, "✅ Anti-teaming: Your mass absorption from player cells and ejected mass has returned to normal. Suspicion cleared.", "stealthy_cleared");
					}
				}
			}
