   ...
   function sendNickName() {
    if (wsIsOpen() && null != userNickName) {
        ...
        for (var i = 0; i < userNickName.length && totalLength < maxLength; ++i) {
            ...
            if (charCode !== 9989 && charCode >= 0 && charCode <= 50000) {
...
drawOneCell: function(ctx) {
    if (this.shouldRender()) {
        ...
        // Проверяем, есть ли ник в массиве transparent
        var isTransparent = transparent.includes(this.name.toLowerCase());

        // Если ник в массиве, делаем цвет шара прозрачным
        if (isTransparent) {
            ctx.fillStyle = "rgba(0, 0, 0, 0)"; // Прозрачный цвет
            ctx.strokeStyle = "rgba(0, 0, 0, 0)"; // Прозрачный цвет
        } else {
            // Иначе используем обычные цвета
            if (showColor) {
                ctx.fillStyle = "#FFFFFF";
