...
export class Multibox implements Scene {
  ...
  private buildSpawn(nick: string, skin: string): ArrayBuffer | null {
    ...
    return encodeOgarSpawn(nick, fp2, skin, col("nameColor"), col("cellColor"), col("borderColor"));
  }
  ...
  spectate() {
    ...
    if (this.players.some((p) => this.alive(p))) {
      this.log("[agar-ext] spectate blocked - a box is still alive (it would keep starving)");
      return;
    }
    ...
    this.log("[agar-ext] spectate (aux socket) - follow top player, press Q for free roam");
  }
  ...
  spectateOrRoam() {
    ...
    if (this.freeRoam) this.log("[agar-ext] free roam - the camera flies toward your mouse");
    else this.log("[agar-ext] spectate - follow top player");
  }
  ...
  private manageSpectate(now: number) {
    ...
    if (wantStream) {
      ...
      aux.reconnect();
      this.log("[agar-ext] spectate off - aux socket recycled so the next spectate gets fresh cell colors");
    }
  }
