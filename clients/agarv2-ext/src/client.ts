...
export class SocketClient {
  ...
  async connect() {
    ...
    try {
      ...
      this.log(`[agar-ext] ${this.label}: token mint failed`, e);
      return;
    }
    this.log(`[agar-ext] ${this.label}: connecting`, this.url.slice(0, 80), `proto=${token.slice(0, 13)}`);
    const WS = (window as unknown as { __agarNativeWS?: typeof WebSocket }).__agarNativeWS ?? window.WebSocket;
    let ws: WebSocket;
    try {
      ws = new WS(this.url, token);
    } catch (e) {
      this.log(`[agar-ext] ${this.label}: connect threw`, e);
      return;
    }
    ...
    ws.addEventListener("open", () => {
      this.log(`[agar-ext] ${this.label}: open - replaying ${this.handshake.length} handshake packet(s)`);
    ...
    ws.addEventListener("message", (ev: MessageEvent) => {
      ...
      try {
        ...
        this.log(`[agar-ext] ${this.label}: decode error`, err, hex(ev.data));
      }
    });

    ws.addEventListener("close", (ev: CloseEvent) => {
      if (this.ws === ws) this.open = false;
      this.log(`[agar-ext] ${this.label}: closed code=${ev.code} reason=${ev.reason || "-"}`);
    });
